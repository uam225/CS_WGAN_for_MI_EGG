...
class Generator(nn.Module):
    def __init__(self, noise_dim=100, channels=3, time_steps=1000):
...
class Discriminator(nn.Module):
    def __init__(self, channels, time_steps=1000):
        super(Discriminator, self).__init__()
        self.channels = channels
        self.time_steps = time_steps

        self.model = nn.Sequential(
            nn.Linear(channels*time_steps, 1024),
...
class WGAN:
    def __init__(self, channels, batchsize, noise_dim):
        ...
        self.generator = Generator(noise_dim=noise_dim, channels=channels).to(self.device)
        self.discriminator = Discriminator(channels=channels).to(self.device)

        self.optimiser_G = optim.Adam(self.generator.parameters(), lr=0.00001, betas=(0.1, 0.999))
